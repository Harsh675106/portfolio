...
const PROJECTS = [
  {
    name: "MedCoreAI",
    desc: "Built an AI-driven diagnostic dialogue system for symptom-based triage , follow-up questioning, medical image analysis, report understanding, chat history, and file uploads.",
    status: "Completed",
    link: "https://github.com/Harsh675106/Diagnose-System",
    imgBase: "/MedCoreAi",
    imageFormats: ["png"],
    stack: ["Next.js", "TypeScript", "FastAPI", "PyTorch", "PostgreSQL", "Prisma"],
  },
  {
  ...
  },
  {
    ...
    status: "Completed",
    link: "https://github.com/Harsh675106/Shatranj",
    imgBase: "/shatranj",
  ...
  },
  {
    ...
    status: "Completed",
    link: "https://github.com/Harsh675106/Noteshelf",
    imgBase: "/startimg",
...
const ProjectImage = memo(function ProjectImage({
  ...
  const fallbackFormat = formats.includes("png") ? "png" : formats[formats.length - 1];

  return (
    <picture>
      {formats.includes("avif") && <source srcSet={`${base}.avif`} type="image/avif" />}
      {formats.includes("webp") && <source srcSet={`${base}.webp`} type="image/webp" />}
      <img
        src={`${base}.${fallbackFormat}`}
        alt={alt}
        loading={loading}
        decoding={decoding}
        width={width}
...
HoloCard.propTypes = {
  project: PropTypes.shape({
    ...
    imageFormats: PropTypes.arrayOf(PropTypes.string),
...
const ProjectRail = memo(function ProjectRail({ projects, isDark, activeIndex, onSelect }) {
  ...
  return (
    <div className="relative mt-10">
      ...
      <div className="rail-scroll hide-scrollbar" data-native-scroll>
...
const Projects = memo(function Projects({ isDark }) {
  ...
  useEffect(() => {
    ...
    const sources = PROJECTS.flatMap((project) =>
      (project.imageFormats ?? ["avif", "webp"])
        .filter((format) => format !== "png")
        .map((format) => `${project.imgBase}.${format}`)
    );
  ...
  return (
    ...
    >
      ...
      <div className="relative mx-auto mt-14 max-w-7xl px-4 sm:px-6">
